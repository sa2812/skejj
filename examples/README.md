...
npx tsx src/index.ts check examples/roast-chicken.json

# Solve and display the timed plan
npx tsx src/index.ts make examples/roast-chicken.json
```

---

## roast-chicken.json -- Roast Chicken Dinner
...
## birthday-party.json -- Kids Birthday Party
...
```bash
npx tsx src/index.ts make examples/birthday-party.json
```

---

## london-sightseeing.json -- London Weekend Sightseeing
...
```bash
npx tsx src/index.ts make examples/london-sightseeing.json
```
...
See [`docs/SCHEMA.md`](../docs/SCHEMA.md) for a full field reference, or add a `$schema` key to your JSON file for editor autocomplete:
